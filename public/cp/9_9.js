...
function updateVis() {
    ...
    colorScale.domain([d3.min(dataset, d => d.value), d3.max(dataset, d => d.value)])
    ...
    bars.enter()
        ...
        .attr("fill", function (d) {
            return "rgb(" + (255 - colorScale(d.value)) + ", 0, " + colorScale(d.value) + ")";
        })
        .merge(bars) //Update…
        .transition()
        .duration(500)
    ...
    d3.selectAll("rect")
        .on("click", (d, i, nodes) => {
            console.log("Attempting to remove " + JSON.stringify(d));
            dataset = dataset.filter(x => x.key != d.key);
            console.dir(dataset);
            updateVis();
        })
}
...
d3.selectAll("p")
    .on("click", (d, i, nodes) => {
        if (d3.select(nodes[i]).classed("addDataP")) {
            ...
            dataset.push({
                key: dataset.length > 0 ? dataset[dataset.length - 1].key + 1 : 1,
                value: newNumber
            });
